...
public:
	...
	void saveBoardUI(const std::vector<SDL_FRect>& rects, const std::vector<std::string>& strings, int size, SDL_Color color) noexcept; // 保存棋盘 UI 纹理，供结束后 win/lose 界面使用，避免重复渲染棋盘
	void renderBoardUI() const noexcept; // 渲染保存的棋盘 UI 纹理，供结束后 win/lose 界面使用

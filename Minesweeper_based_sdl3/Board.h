...
public:
    ...
    void resetBoard() noexcept;// 重置棋盘尺寸与地雷数

	void reveal(int r, int c) noexcept; // 打开格子；如果越界或已打开直接返回，否则更新状态并递归打开周围格子
	void doubleClickReveal(int r, int c) noexcept; // 单双击都可，如果格子已打开且周围标记数量等于数字，打开周围未标记的格子，后续泛洪打开
    void toggleFlag(int r, int c) noexcept;// 切换标记
	...
	static constexpr uint8_t maskNum = 0b00001111;//低四位存相邻地雷数（单元格数字）
private:
	...
	int index(int r, int c) const noexcept { return r * cols_ + c; }// 计算行列对应的一维索引
...
private:
	...
	mutable std::vector<transInfo> dirtyCells_; // 需要渲染的格子列表（脏格子）,mutable允许在 const 方法中修改，便于记录需要渲染的格子
    std::mt19937 rng_;// 将随机引擎作为成员，符合 RAII 与可控性，便于测试
};